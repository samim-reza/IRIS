...
> **Presentation pass (2026-09-22, evening).** Manuscript restructured to
> journal form: numbered references (`elsarticle-num-names` -- the numeric
> style that still lets `\citet` print author names; plain `elsarticle-num`
> does not and every `\citet` collapses to a bare `[n]`), the multi-page
> related-work longtable replaced by a one-page comparison matrix, the 60-row
> results table split into two one-page wide tables (`results/summary_final.tex`,
> `summary_aubc.tex`, generated by `make_plots.py`), two new results tables
> (reliability-gate transfer, component ablations) built from macros, every
> float on `pos=htbp`, every float cited, second author added. Watch for:
> the generic `BestBase*`/`IrisFinalGain` macros compare against BADGE now;
> prose about the hand-crafted family must use the `*Classic*` macros and
> the BADGE gap the `BadgeOverIris*` ones.
>
